...
var obs = new _rxjs.Observable(function (subscriber) {
  ...
  var sub = setInterval('console.log("leake");', 1000); // subscriber.complete()
...
}, 2000); //example for interval
// this will emit the data per second

var inter = (0, _rxjs.interval)(1000);
var interObj = inter.subscribe(console.log);
setTimeout(function () {
  console.log('closed intervel operator');
  interObj.unsubscribe();
}, 4000); //timer operator example

var timerDemo = (0, _rxjs.timer)(1000, 1000);
var timerObj = timerDemo.subscribe(console.log);
setTimeout(function () {
  console.log('timer operator will terminated');
  timerObj.unsubscribe();
}, 4000); //from event operator demo

var fromEventListener = (0, _rxjs.fromEvent)(document, 'click');
var fromEventListenerObj = fromEventListener.subscribe(console.log); //fromEventListenerObj.unsubscribe();
//of operator example

var ofDemo = new _rxjs.of([1, 2], [2, 3, 4], [10, 20, 30, 40]);
var ofDemoObj = ofDemo.subscribe({
  next: function next(response) {
    console.log('of: ' + response);
  }
});
ofDemoObj.unsubscribe(); //for operator 

var fromDemo = new _rxjs.from([1, 2, 3, 4]);
var fromObj = fromDemo.subscribe({
  next: function next(result) {
    console.log('from: ' + result);
  }
}); //demo for the map operator

var ofObservable = new _rxjs.of(1, 2, 3, 4, 5);
var firstPipableObservable = ofObservable.pipe((0, _operators.map)(function (value) {
  return "$".concat(value);
}));
var mapOperator = firstPipableObservable.subscribe({
  next: function next(result) {
    console.log('map: ' + result);
  }
}); //pluck operator demo

var fromEventObserver = new _rxjs.fromEvent(document, 'keydown').pipe((0, _operators.pluck)('code'));
var pluckSubscriber = fromEventObserver.subscribe({
  next: function next(result) {
    console.log('pluck: ' + result);
  }
}); //filter operator demo

var filterObservable = new _rxjs.of(1, 2, 3, 4, 10, 6, 20, 40).pipe((0, _operators.filter)(function (val) {
  return val >= 6;
}));
var filterSubscriber = filterObservable.subscribe({
  next: function next(result) {
    console.log('filter: ' + result);
  }
}); //reduce operator demo

var reduceObservable = new _rxjs.of(1, 2, 3, 4, 5).pipe((0, _operators.reduce)(function (acc, val) {
  return acc + val;
}, 0));
var reduceSubscriber = reduceObservable.subscribe({
  next: function next(result) {
    return console.log('reduce :' + result);
  }
});
